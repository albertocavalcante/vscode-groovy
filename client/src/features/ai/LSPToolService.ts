...
export class LSPToolService implements ILSPToolService {
  ...
  public async findWorkspaceSymbol(query: string): Promise<AISymbolInfo[]> {
    ...
    return symbols.map((s: vscodeType.SymbolInformation) =>
      this.toAISymbolInfo(s),
    );
  }
  ...
  public async findReferences(
    ...
    const locations = await this.vscode.commands.executeCommand<
      vscodeType.Location[]
  ...
  public async getDefinition(
    ...
    const result = await this.vscode.commands.executeCommand<
      vscodeType.Location | vscodeType.Location[] | vscodeType.LocationLink[]
    ...
    return {
      ...
      range: {
        start: {
          ...
          line: link.targetRange.end.line,
          character: link.targetRange.end.character,
        },
      },
    };
  }
