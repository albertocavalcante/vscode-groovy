...
suite("Extension Test Suite", () => {
  ...
  test("Should register all Groovy commands", async () => {
    ...
    // Core commands
    assert.ok(
      commands.includes("groovy.restartServer"),
      "Should register restart server command",
    );
    assert.ok(
      commands.includes("groovy.checkForUpdates"),
      "Should register check for updates command",
    );

    // Status bar commands
    assert.ok(
      commands.includes("groovy.showStatusMenu"),
      "Should register show status menu command",
    );
    assert.ok(
      commands.includes("groovy.openLogs"),
      "Should register open logs command",
    );
    assert.ok(
      commands.includes("groovy.stopServer"),
      "Should register stop server command",
    );
    assert.ok(
      commands.includes("groovy.reportIssue"),
      "Should register report issue command",
    );

    // Gradle commands
    assert.ok(
      commands.includes("groovy.gradle.build"),
      "Should register gradle build command",
    );
    assert.ok(
      commands.includes("groovy.gradle.test"),
      "Should register gradle test command",
    );
    assert.ok(
      commands.includes("groovy.gradle.clean"),
      "Should register gradle clean command",
    );
  ...
  test("Configuration should have correct structure", () => {
    ...
    assert.ok(
      ...
      "Should have trace.server configuration",
    );
    assert.ok(
      config.has("compilation.mode"),
      "Should have compilation.mode configuration",
    );
  });
  ...
  test("Status bar configuration should have correct structure", () => {
    ...
    assert.ok(
      config.has("statusBar.show"),
      "Should have statusBar.show configuration",
    );
    ...
    assert.ok(
      config.has("statusBar.clickAction"),
      "Should have statusBar.clickAction configuration",
    );
  ...
  test("Should use correct output channel", async () => {
    ...
    assert.strictEqual(
      client.outputChannel.name,
      "Groovy Language Server",
      "Output channel name should match",
    );
  });
});
