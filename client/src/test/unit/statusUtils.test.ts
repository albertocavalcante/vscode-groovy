...
describe("Status Utils", () => {
  ...
  describe("inferStateFromMessage", () => {
    ...
    it("should infer degraded from error message", () => {
      assert.strictEqual(
        inferStateFromMessage("Something failed", undefined),
        "degraded",
      );
    });
    ...
    it("should infer resolving-deps", () => {
      assert.strictEqual(
        inferStateFromMessage("Resolving Gradle dependencies", undefined),
        "resolving-deps",
      );
    });
    ...
    it("should infer indexing", () => {
      assert.strictEqual(
        inferStateFromMessage("Indexing 50 files", undefined),
        "indexing",
      );
    });
    ...
    it("should infer ready", () => {
      assert.strictEqual(
        inferStateFromMessage("Service Ready", undefined),
        "ready",
      );
    });
    ...
    it("should return undefined for unknown messages", () => {
      assert.strictEqual(
        inferStateFromMessage("Unknown state", undefined),
        undefined,
      );
    });
  });
